...
def _set_session_persistence(session_id: str):
    """Set session persistence using query parameters as fallback."""
    if _st is None:
        return
    ...
    # Use query parameters as a fallback for refresh persistence
    # This is less secure but works reliably across refreshes
    _st.query_params["sid"] = session_id

def _get_session_persistence() -> Optional[str]:
    """Get session from query parameters or session state."""
    if _st is None:
        return None
    ...
    session_id = _st.query_params.get("sid")
    if session_id:
...
def _clear_session_persistence():
    ...
    if "sid" in _st.query_params:
        # Note: We can't directly modify query_params, but the session will be invalid anyway
        pass
...
def get_current_user() -> Optional[str]:
    ...
    # Try to get session from persistence (query params or session state)
    session_id = _get_session_persistence()
    if not session_id:
        return None
        ...
        with st.form("login_form"):
            ...
            if login_button:
                if not user_id or not password:
                    ...
                    _set_session_persistence(session_id)
                    _st.rerun()
                else:
